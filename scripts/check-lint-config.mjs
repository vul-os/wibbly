 ...
 *
 * ---------------------------------------------------------------------------
 * WIBBLY FORK NOTE (this file is the repo-root copy): wibbly has TWO ESLint
 * configs in one repo — this root eslint.config.js (npm workspace root:
 * src/, games/tennis, packages/*) and games/palmworks/eslint.config.js, a
 * SEPARATE npm project (own package.json/package-lock.json/node_modules)
 * outside the workspaces, excluded from this config via
 * globalIgnores(['games/palmworks', ...]). This copy proves the root config;
 * games/palmworks/scripts/check-lint-config.mjs proves palmworks
 * independently, since it is a different ESLint install with a different
 * tsconfig.json.
 *
 * `npm run typecheck` chains through both
 * (`... && npm run typecheck --prefix games/palmworks`) — that chaining is
 * load-bearing (palmworks once accumulated 668 unnoticed type errors
 * precisely because it had no typecheck script reachable from the root), and
...
const CONFIG = {
  ...
  // Directory (relative to webRoot) that tsconfig.json's "include" covers
  // (`["src", "test", "games/tennis"]`), so the type-awareness probe fixture
  // lands somewhere `projectService` actually resolves.
  fixtureDir: 'src',
  fixtureName: '__lint_config_gate_probe.ts',

  // Repo root to walk for package.json files (assertion B). This also picks
  // up games/palmworks/package.json and packages/*/package.json in the same
  // sweep, so running this assertion from BOTH copies (root and palmworks)
  // is intentionally redundant: either alone would already catch a bad pin
  ...
  // (assertion C). Measured today: `eslint .` from the repo root lints 82
  // files (0 errors, 25 warnings).
  coverageFloor: 65,
}
