...
import { handleBallHit } from './ball';
import { calculateOptimalPosition } from './player';
import type { BallObject, GameState, PlayerData, PlayerObject } from './types';

/** Minimal clock surface `updatePlayerPositions` reads — satisfied by `THREE.Clock` or a test stub. */
export interface PositionsClock {
...
export function updatePlayerPositions(gameState: GameState, playerData: PlayerData[], clock: PositionsClock): void {
...
export function updatePlayer1AI(
...
export function updatePlayer2AI(
    players: PlayerObject[],
    gameState: GameState,
    playerData: PlayerData[],
    ballGroup: BallObject,
