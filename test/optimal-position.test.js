...
import { calculateOptimalPosition } from '../src/game/player.js';
import { createGameState } from '../src/game/game-logic.js';
...
// stubs suffice.
const ball = (x, y, z) => ({ position: { x, y, z } });
const player = (x, z) => ({ position: { x, z } });

function inPlay(vx, vy, vz) {
...
describe('calculateOptimalPosition', () => {
  ...
  it('is pure — identical inputs give identical output', () => {
    const args = [player(-6, 0), ball(-2, 2, 1), inPlay(-3, -1, 2), 0];
