...
|---|---|---|
| `packages/wibbly-input` library | **Built** | TypeScript, zero DOM injection, 231 unit tests passing without a camera |
...
| **Hand tracking** (MediaPipe HandLandmarker) | **Built, composed into a game** | `HandLandmarkTracker` exists and is unit-tested, including distance-from-camera and rotation invariance — but thresholds are still derived from geometry, not a real hand, since no hand-tracking session has run against a live camera yet. It **is** now composed into a real pipeline: `HandInput` (`packages/wibbly-input/src/hand-pipeline.ts`, exported at the new `@vulos/wibbly-input/hand` subpath) drives [`games/palmworks`](games/palmworks)'s `GestureController` → `VirtualPointer`, which dispatches real DOM pointer events so `PlantScene`'s existing click/drag handlers do the hit-testing — 46 new tests exercise this against synthetic `Hand` fixtures. Wibbly's own body-pose pipeline (`pipeline.ts`, what tennis uses) is untouched and still doesn't compose the hand tracker — see Palmworks, below, for why that's still not the same as "reachable in wibbly." Its assets (the `.task` model + MediaPipe Wasm runtime, ~40 MiB) vendor same-origin on demand via `npm run vendor:hands`; the tracker already defaults to those vendored paths, so `init()` works once it's run. Not committed to git: `games/palmworks`'s copy was committed once and then stripped back out of history with `git-filter-repo`, so a bare clone has to run the script before hand tracking will initialise there. |
...
| **Palmworks** | **App builds, gestures wired, not a wibbly game yet** | [`games/palmworks`](games/palmworks) is a real, independent React app — its own routes, its own build, playable standalone by mouse and keyboard, no auth, no backend. It is folded into this repo with its full history. It is **not reachable from wibbly's own title screen**: `src/components/catalogue.ts` lists it with `status: 'planned'`, so it renders as a non-clickable **Planned** card on [`src/pages/title.jsx`](src/pages/title.jsx), exactly like Soccer and Boxing. It now has **real gesture wiring**, not zero: `HandLandmarkTracker`/`PinchRecognizer`/`PointRecognizer` (see Hand tracking, above) drive this game's own `GestureController` and `VirtualPointer` so pinch-to-place, point-to-select and pinch-tap-to-connect all work, covered by 46 tests against synthetic hand fixtures. What hasn't changed: no session has run it against a live camera, and this title-screen card still can't be clicked into it — see `games/palmworks/README.md` for how to run it directly, including the `npm run vendor:hands` step the gesture path needs. |
