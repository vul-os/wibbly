...
interface RackSystemProps extends PlantObjectProps {
  position: [number, number, number];
}

interface RackSystemPort {
  id: string;
...
const RackSystem: PlantObjectComponent<RackSystemProps, RackSystemPort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  ...
  const connectionPorts: RackSystemPort[] = [
    {
      id: 'conveyor_in',
      type: 'liquid',
  ...
  const handlePortClick = (port: RackSystemPort, event: ThreeEvent<MouseEvent>) => {
    event.stopPropagation();
    if (onPortClick) {
  ...
  const getPortColor = (port: RackSystemPort): string => {
    switch (port.type) {
