...
interface HeatExchangerProps extends PlantObjectProps {
  position: [number, number, number];
}

interface HeatExchangerPort {
  id: string;
...
const HeatExchanger: PlantObjectComponent<HeatExchangerProps, HeatExchangerPort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  ...
  const connectionPorts: HeatExchangerPort[] = [
    {
      id: 'electric_in',
      type: 'electric',
  ...
  const handlePortClick = (port: HeatExchangerPort, event: ThreeEvent<MouseEvent>) => {
    event.stopPropagation();
    if (onPortClick) {
  ...
  const getPortColor = (port: HeatExchangerPort): string => {
    switch (port.type) {
