...
interface BoilerProps extends PlantObjectProps {
  position: [number, number, number];
}

interface BoilerPort {
  id: string;
...
const Boiler: PlantObjectComponent<BoilerProps, BoilerPort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  ...
  // Define connection ports for the boiler
  const connectionPorts: BoilerPort[] = [
    {
      id: 'electric_in',
      type: 'electric',
      label: 'Power Input',
  ...
  const handlePointerDown = (event: ThreeEvent<PointerEvent>) => {
    ...
    const handlePointerUp = () => {
      ...
      if (!hasMovedMouse) {
        ...
        // Same handler reused for touch events — it only ever reads
        // `.clientX`/`.clientY`, which a bare `TouchEvent` doesn't carry (that
        // would need `.touches[0]`), so touch-dragging was already inert
        // before this migration. The cast preserves that as-is; fixing it is
        // a behavior change, out of scope here.
    ...
    // Prevent default to avoid text selection. `ThreeEvent` (the R3F
    // synthetic pointer event) never had a `preventDefault` — only
    // non-function properties are copied from the native event — so this
    // was already a no-op before this migration; the cast just lets us ask
    // the same (always-false) question under strict typing.
    (event as unknown as { preventDefault?: () => void }).preventDefault?.();
  };

  const handlePortClick = (port: BoilerPort, event: ThreeEvent<MouseEvent>) => {
    event.stopPropagation();
    if (onPortClick) {
  ...
  const getPortColor = (port: BoilerPort): string => {
    switch (port.type) {
  ...
  return (
    ...
    >
      ...
      <mesh position={[0, 1.6, 0]} castShadow>
        ...
        <meshLambertMaterial color="#357a35" />
      </mesh>
