...
type TabId = 'controls' | 'camera' | 'settings' | 'help';
 ...
 * for it.
 */
const PlannedRow = ({ label, note, children }: PlannedRowProps) => (
...
export interface InGameMenuProps {
  ...
  onSettingsChange?: (key: keyof GameSettings, value: boolean) => void;
...
const InGameMenu = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // `Element` has no `.focus` — only `HTMLElement`/`SVGElement` do — so the
