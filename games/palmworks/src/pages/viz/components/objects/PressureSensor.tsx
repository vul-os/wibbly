...
interface PressureSensorProps extends PlantObjectProps {
  position: [number, number, number];
}

interface PressureSensorPort {
  id: string;
...
const PressureSensor: PlantObjectComponent<PressureSensorProps, PressureSensorPort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  const groupRef = useRef<THREE.Group>(null);
  const [isDragging, setIsDragging] = useState(false);
  const [hovered, setHovered] = useState(false);
  const [hoveredPort, setHoveredPort] = useState<string | null>(null);
  const [, setCurrentPressure] = useState(12.7);
  ...
  const connectionPorts: PressureSensorPort[] = [
    {
      id: 'power_24vdc_loop',
      type: 'electric',
  ...
  const getPortColor = (port: PressureSensorPort): string => {
    switch (port.type) {
