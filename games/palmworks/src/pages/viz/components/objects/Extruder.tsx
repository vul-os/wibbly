...
interface ExtruderProps extends PlantObjectProps {
  position: [number, number, number];
}

interface ExtruderPort {
  id: string;
...
const Extruder: PlantObjectComponent<ExtruderProps, ExtruderPort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  ...
  const connectionPorts: ExtruderPort[] = [
    {
      id: 'electric_motor',
      type: 'electric',
      label: 'Motor Power',
  ...
  const handlePortClick = (port: ExtruderPort, event: ThreeEvent<MouseEvent>) => {
    event.stopPropagation();
    if (onPortClick) {
  ...
  const getPortColor = (port: ExtruderPort): string => {
    switch (port.type) {
