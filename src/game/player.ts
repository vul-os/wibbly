...
import type { BallObject, GameState, PlayerData, PlayerObject } from './types';

export function createPlayer(x: number, z: number, rotation: number, scale = 1): PlayerObject {
    ...
    return group as PlayerObject;
}

export function updatePlayerMovement(player: PlayerObject, data: PlayerData, delta: number): void {
...
export function updatePlayerSwing(player: PlayerObject, data: PlayerData, delta: number, playerIndex: number): void {
    if (data.swinging) {
...
export function updateRacketAlignment(
    player: PlayerObject,
    data: PlayerData,
...
/** Anything with an `.x`/`.z` position — a real `PlayerObject`/`THREE.Group`, or a plain test stub. */
export interface HasXZPosition {
    position: { x: number; z: number };
}

/** Anything with an `.x`/`.y`/`.z` position — a real `BallObject`/`THREE.Group`, or a plain test stub. */
export interface HasXYZPosition {
...
export function calculateOptimalPosition(
    player: HasXZPosition,
    ballGroup: HasXYZPosition,
    gameState: GameState,
    playerIndex: number,
): OptimalPosition {
...
export function toggleHitBoxVisibility(
    ...
    ballGroup: BallObject | null | undefined,
    ...
    // Narrowed to a definite boolean above; captured as a `const` so the
    // forEach closure below keeps that narrowing instead of widening back to
    // `boolean | null`.
    ...
    players.forEach((player) => {
        if (player.userData.hitBox) {
            player.userData.hitBox.visible = shouldShow;
        }
    });

    // Toggle ball collision sphere
    if (ballGroup && ballGroup.userData.collisionSphere) {
        ballGroup.userData.collisionSphere.visible = shouldShow;
    }

    debugLog(`Hit boxes ${shouldShow ? 'shown' : 'hidden'}`);
    return shouldShow;
}