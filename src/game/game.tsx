...
import { PeerSession, type PeerTransport } from '@vulos/wibbly-p2p';
...
import { loadCourt } from './court';
import { createBall, updateBallPhysics, handleBallHit } from './ball';
import {
    createGameState,
    createPlayerData,
    updatePlayer1AI,
 ...
 */
function peerTransport(): PeerTransport | null {
    // resolvePeerTransport() only ever hands back what a host page assigned to
    // `window.__WIBBLY_PEER_TRANSPORT__` (or null) — see mode.ts's own doc
    // comment on that global for the contract. It is typed `unknown` there
...
/**
 * A `THREE.Object3D` that may carry the geometry/material properties its
 * renderable subclasses (`Mesh`, `Points`, `Line`, …) actually have. `traverse`
 * only guarantees the base `Object3D` shape, so this is how the disposal walk
 * below reaches into whichever of those a given node happens to be without
 * assuming which subclass it is.
 */
...
/** `String(err && err.message ? err.message : err)`, typed for `unknown` catch bindings. */
function errorMessage(err: unknown): string {
    const withMessage = err as { message?: unknown } | null | undefined;
    return String(withMessage && withMessage.message ? withMessage.message : err);
}
 ...
 */
function disposeObject3D(root: THREE.Object3D | null | undefined): void {
    if (!root) return;
    root.traverse((obj) => {
        const node = obj as MaybeRenderable;
 ...
 */
export interface TennisGameProps {
    paused?: boolean;
    settings?: GameSettings | null;
    calibration?: Calibration | null;
    onInputState?: ((status: 'live' | 'keyboard') => void) | null;
    onSwing?: ((direction: SwingDirection) => void) | null;
    onTrackerBackend?: ((info: BackendInfo) => void) | null;
    onAuthority?: ((telemetry: AuthorityTelemetry) => void) | null;
}
...
function TennisGame({
    ...
    const playersRef = useRef<PlayerObject[]>([]);
    ...
    const authorityRef = useRef<MagnetiteAuthorityRunner | null>(null);
    ...
    const [input, setInput] = useState<WibblyInput | null>(null);
    const [trackedPlayers, setTrackedPlayers] = useState<PlayerId[]>([]);

    useEffect(() => {
        if (!containerRef.current) return;
        // Captured once, right after the null check: `containerRef.current`
        // is read again much further down (after many intervening statements
        // and function declarations), too far for TypeScript to keep treating
        // it as non-null just from this guard.
        ...
        function updateWiiSportsCamera(player1: PlayerObject, ballGroup: BallObject, gameState: GameState): void {
        ...
        function handleSwing(swingDirection: SwingDirection = 'right', confidence = 1): void {
        ...
        function animate(): void {
            ...
            if (authorityRef.current) {
                try {
                    ...
                    window.__WIBBLY_MAGNETITE__ = { ready: false, error: errorMessage(err) };
                }
            }
