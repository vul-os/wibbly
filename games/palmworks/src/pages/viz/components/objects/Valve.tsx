...
interface ValveProps extends PlantObjectProps {
...
const Valve: PlantObjectComponent<ValveProps, ValvePort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  ...
  // Define connection ports for the valve
  const connectionPorts: ValvePort[] = [
    {
      id: 'electric_control',
      type: 'electric',
  ...
  const handlePortClick = (port: ValvePort, event: ThreeEvent<MouseEvent>) => {
    event.stopPropagation();
    if (onPortClick) {
  ...
  const getPortColor = (port: ValvePort): string => {
    switch (port.type) {
