...
interface DistillationColumnProps extends PlantObjectProps {
  position: [number, number, number];
}

interface DistillationColumnPort {
  id: string;
...
const DistillationColumn: PlantObjectComponent<DistillationColumnProps, DistillationColumnPort> = ({ position, onClick, onDrag, isSelected, isDraggable, gridSnap, gridSize, onPortClick }) => {
  ...
  const connectionPorts: DistillationColumnPort[] = [
    {
      id: 'electric_in',
      type: 'electric',
      label: 'Power Input',
  ...
  const handlePortClick = (port: DistillationColumnPort, event: ThreeEvent<MouseEvent>) => {
    event.stopPropagation();
    if (onPortClick) {
  ...
  const getPortColor = (port: DistillationColumnPort): string => {
    switch (port.type) {
